...
// ##**##** NPFL CODE BLOCK 1 START ##**##** 
	...
	// initial results query, before user inputs
	$query_apt = "SELECT * from apartments, buildings, neighborhoods WHERE
	  apartments.bldgID = buildings.IDbldg AND
	  buildings.hoodID = neighborhoods.IDhood";

	$orderBy = 'rent'; // how user wants to sort results
	$ascDesc = 'ASC'; // radio button choice "ASC" or "DESC"

	// if the search form is submitted, filter the results
	if(isset($_GET['bldgID'])){
		//if there is a value for building, even 'ANY', means form was submitted
	...
	$query_apt .= " AND rent BETWEEN '$minRent' AND '$maxRent'";

	// concat query if user chose a bldg from dynamic bldg menu
	if($bldgID != -1) { // if user chose a bldg (not ANY)
		$query_apt .= " AND bldgID='$bldgID'";
	}

	// concat query if user typed something into search box
	if(isset($_GET['search']) && $_GET['search'] != "") { // true if user typed something
		$search = $_GET['search'];
		$query_apt .= " AND (aptDesc LIKE '%$search%'
	...
	if(isset($_GET['doorman'])) { // is the doorman variable set. if so it came over from the form, meaning doorman was checked
		$query_apt .= " AND isDoorman=1";
	}

	if(isset($_GET['pets'])) { 
		$query_apt .= " AND isPets=1";
	}

	if(isset($_GET['parking'])) { 
		$query_apt .= " AND isParking=1";
	}

	if(isset($_GET['gym'])) { 
		$query_apt .= " AND isGym=1";
	}

	if(isset($_GET['isAvail'])) { 
		$query_apt .= " AND isAvail=1";
	}

	}// end of if form submtted

	$query_apt .= " ORDER BY $orderBy $ascDesc"; // this line MUST be LAST !
	...
	$query_limit = sprintf("%s LIMIT %d, %d", $query_apt, $startRow, $rowsPerPg);

	$result_apt = mysqli_query($conn, $query_limit);  // the result will be an array of arrays (or, a multi-dimensional array)

	 if(isset($_GET['totalRows'])) { // true only if not on first page
		  $totalRows = $_GET['totalRows'];
		} else {
		  $all = mysqli_query($conn, $query_apt);
